...
router.post("/bots/:id/approve", async (req, res) => {
 let bot = await botModel.findOne({ id: req.params.id });
  if (!bot || bot.deleted) return res.render("panel/bots.ejs", { message: "This bot could not be found on our list." });
  if (bot.status === "approved" || bot.status === "denied") return res.render("panel/bots.ejs", { message: "This bot has already been approved or denied." });
  ...
  await bot.save().then(async () => {
    ...
    res.render("panel/bots.ejs", { message: "Successfully Approved", code: "OK" });
    let logs = client.channels.get(config.channels.weblogs);
    logs.sendMessage(
      ...
      bot.owners.forEach(async (owner) => {
        ...
        await client.api.get(`/servers/${config.servers.main}/members/${owner}`).then(async (data) => {
          ...
          await sleep(700)
          await client.api.patch(`/servers/${config.servers.main}/members/${owner}`, { "roles": roles }).catch(() => { return res.render("panel/bots.ejs", { message: `Unable to add role to this user.` }) });
        });
...
router.post("/certification/:id/deny", async (req, res) => {
  ...
  await bot.save().then(async () => {
    try {
      res.render("panel/certification.ejs", { message: "Successfully Denied Certification", code: "OK" });
      let logs = client.channels.get(config.channels.weblogs);
      logs.sendMessage(
...
router.post("/certification/:id/certifyDelete", checkAdmin, async (req, res) => {
  ...
  await bot.save().then(async () => {
    try {
      res.render("panel/certification.ejs", { message: "Successfully Deleted Certification", code: "OK" });
      let logs = client.channels.get(config.channels.weblogs);
      logs.sendMessage(
